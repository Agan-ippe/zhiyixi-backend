...
import com.zhiyixi.model.dto.user.*;
...
import static com.zhiyixi.service.impl.UserServiceImpl.SALT;

/**
 * @Author <a href="https://github.com/Agan-ippe">知莫</a>
 * @Date 2025/09/08   13:17
 * @Version 1.0
...
public class UserController {
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<UserDO> getUserById(long id, HttpServletRequest request) {
        if (id <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
     ...
     */
    @GetMapping("/get/vo")
    @ApiOperation(value = "根据 id 获取用户封装类")
    public BaseResponse<UserVO> getUserVOById(long id, HttpServletRequest request) {
        BaseResponse<UserDO> response = getUserById(id, request);
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Page<UserDO>> listUserByPage(@RequestBody UserQueryRequest userQueryRequest,
                                                   HttpServletRequest request) {
        long current = userQueryRequest.getCurrent();
        long size = userQueryRequest.getPageSize();
        Page<UserDO> userPage = userService.page(new Page<>(current, size),
